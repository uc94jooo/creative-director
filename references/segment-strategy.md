# 策略段 (Strategy Segment) — Phase 1 / 2 完整流程

> 本檔為策略段的完整操作細節。進入策略段（開始解讀需求與設計對齊表）的第一動作＝Read 本檔。
> 段落結案物＝三件套：對齊表明檔 vX ＋ insight-pass 串 ＋ dialogue-log。
> 結案後停住，明確告知使用者；創意段需使用者下指令才啟動（進場規則見 SKILL.md）。
> 常駐硬閘門（終止 Turn、人工介入點暫停）已列於 SKILL.md，本檔內同名規則為其展開細節。
...
2.  **自主傳播測試 (Organic Share Test)**：受眾是否願意「主動」分享此工具/體驗給朋友，即便品牌沒有提供 any 抽獎或金錢獎勵？
...
- **對齊表存檔步驟**（使用者確認對齊表後，進入下一 Phase 之前，依序執行）：
  1. 檔名固定格式：`01_對齊表_[案名].md`，存在專案子資料夾（與主 MD 同層，不是 `.cd-notes/` 內）。`01` 為產線工件編號前綴（01對齊表→02講稿與解讀→03交棒單→04文案包→05輪播，缺號代表該案未經過該工位），日期記於 frontmatter。**本規則適用於新案；舊專案既有檔名不回溯改名**。
  2. frontmatter 固定四欄：
     ```yaml
     ---
     case: "{案名一句話}"
  ...
  4. 若使用者後續再修正對齊表：不覆寫舊檔，version +1 開新檔，檔名加版本尾碼（例：`01_對齊表_[案名]_v2.md`）。舊版檔案保留不動。
  5. 存檔後向使用者回報檔案路徑。
